    ...
    public class LandingController : Controller
    {
        ...
        public async Task<ActionResult> blog(String generoFilter)
        {
            ...
            try
            {
                ...
                {
                    var response = await client.GetAsync("https://localhost:44377/api/peliculas/inicio");
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        if (res.respuesta)
                        {
                            model.PeliculaInicio.Peliculas = res.Peliculas;

                            if (res.Comentario != null)
                            {
                                
                                model.PeliculaInicio.Comentario = res.Comentario;
                            }
                            else
                            {
            ...
            try
            {
                ...
                var jsonContent = new StringContent("{\"Generos\": {\"IdGenero\":" + longValue + "}}", Encoding.UTF8,
                    "application/json");
                using (HttpClient client = new HttpClient())
                {
